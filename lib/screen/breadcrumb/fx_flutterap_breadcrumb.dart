                ...
                _index.contains(index)
                    ...
                    ? GestureDetector(
                        onTap: () {
                          Get.until(
                            (route) {
                              dynamic arguments = (route as GetPageRoute).settings.arguments;
                              dynamic arguments1 = (value as GetPageRoute).settings.arguments;
                              if (arguments == null || arguments?["path"] == null) {
                                return true;
                              }
                              return arguments?["path"] == arguments1?["path"];
                            },
  ...
  Widget _breadButton(BreadButtonType type, int index) {
    if (widget.breadButtonType == BreadButtonType.simple) {
      return FxSimpleBreadButton(
        index == 0 ? widget.firstRoute ?? "Home" : widget.currentRouteStack[index].settings.name ?? "",
        widget.suffix ??
            Text(
              " / ",
              style: TextStyle(
